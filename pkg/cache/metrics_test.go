...
func Test_sharedCacheFactory_metrics_collection(t *testing.T) {
	...
	cf.EXPECT().ForResourceKind(gvr, "ConfigMap", true).DoAndReturn(func(gvr schema.GroupVersionResource, kind string, namespaced bool) *client.Client {
		return testClient
	})
	...
	err = testutil.CollectAndCompare(metrics.TotalCachedObjects, strings.NewReader(`
# HELP lasso_controller_total_cached_object Total count of cached objects
# TYPE lasso_controller_total_cached_object gauge
lasso_controller_total_cached_object{ctx="",group="",kind="ConfigMap",version="v1"} 0
`))
	if err != nil {
		t.Fatal(err)
	}
